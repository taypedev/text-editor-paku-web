...
import { getBlogs, saveBlog } from "@/repo/blog.controller";
...
export function TextEditor() {
  const [newValue, setNewVakye] = useState<any>([]);

  const createBlog = async () => {
    try {
      await saveBlog(newValue);
      alert("se publico");
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <section className="mt-4">
      <Button onClick={() => createBlog()}>Publicar</Button>
